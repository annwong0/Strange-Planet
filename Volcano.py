...
class Volcano(Mountain):
	...
	def act(self, map):
		...
		if self._countdown == 0: 
			...
			n_cells = map.get_neighbours(self._row, self._col)
			for cell in n_cells:
				if cell != None and cell.occupant != None:
					if cell.occupant.name == 'Player':
						cell.occupant.hp -= 1
					elif cell.occupant.name == 'Goblin':
	...
	def display(self):
		...
		print('%s %s \033[0;0m' %(self._color, self._countdown), sep='', end='   ', flush=True)
        # END TODO 